...
void t_swift_generator::generate_swift_service_server_async(ostream& out, t_service* tservice) {
  ...
  out << '\n';
  out << indent() << "typealias ProcessorHandlerDictionary = "
      << "[String: (Int32, TProtocol, TProtocol, " << tservice->get_name() << "Async) throws -> Void]" << '\n'
      << '\n'
      << indent() << "public var service: " << tservice->get_name() << "Async" << '\n'
      << '\n'
...
void t_swift_generator::generate_swift_service_server_async_implementation(ostream& out,
  ...
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    ...
    if (!tfunction->is_oneway() ) { // oneway does not work properly
      ...
      out << indent() << "} catch let error as TApplicationError";
      block_open(out);
      out << indent() << "_ = try? outProtocol.writeException(messageName: \"" << tfunction->get_name() << "\", sequenceID: sequenceID, ex: error)" << '\n'
          << indent() << "return" << '\n';
      indent_down();
      out << indent() << "} catch let error";
      block_open(out);
      out << indent() << "_ = try? outProtocol.writeException(messageName: \"" << tfunction->get_name() << "\", sequenceID: sequenceID, ex: TApplicationError(error: .internalError))" << '\n'
          << indent() << "return" << '\n';
      block_close(out);
